...
class TestAndroidLatest(object):
    ...
    def setup_class(cls):
        ...
        try:
            ...
            with allure.step("获取测试URI值。"):
                cls.URI = cls.config.getItem('uri', 'AndroidLatest')
    ...
    def test_xxxxxx_android_latest_without_login(self):
        """ Test get android lastest without login(FT-HTJK-xxx-xxx)."""
        self.logger.info(".... Start test_xxxxxx_android_latest_without_login ....")
        try:
            with allure.step("teststep1: requests http get."):
                rsp = self.httpclient.get(self.URI)
                allure.attach("request.headers", str(rsp.request.headers))
                allure.attach("request.url", str(rsp.request.url))
                self.logger.info("request.headers: {}".format(rsp.request.headers))
                self.logger.info("request.url: {}".format(rsp.request.url))

            with allure.step("teststep2: assert the response code"):
            ...
            with allure.step("teststep3: assert the response content"):
                ...
                assert rsp_content['result']['latest_url']
        except Exception as e:
            ...
            self.logger.info(".... End test_xxxxxx_android_latest_without_login ....")
            self.logger.info("")

    @allure.severity("critical")
    @allure.story("已经登录")
    @allure.testcase("FT-HTJK-xxx-xxx")
    def test_xxxxxx_android_latest_with_login(self):
        """ Test get android lastest with login(FT-HTJK-xxx-xxx)."""
        self.logger.info(".... Start test_xxxxxx_android_latest_with_login ....")
        try:
            with allure.step("teststep1: user register."):
                json = {"code_type": 0, "client_type": 1, "client_version": "v1", "device_token": "123456789",
                        "imei": "460011234567890", "phone": "13511222393", "sms_code": "123456",
            ...
            self.logger.info(".... End test_xxxxxx_android_latest_with_login ....")
            self.logger.info("")


if __name__ == '__main__':
    pytest.main(['-s', 'test_APP_AndroidLatest.py'])
    # pytest.main(['-s', 'test_APP_AndroidLatest.py::TestAndroidLatest::test_xxxxxx_android_latest_with_login'])
