...
@allure.feature("BC-业务中心用户身份鉴权")
class TestBCUserAuth(object):
    ...
    def setup_class(cls):
        ...
        try:
            ...
            with allure.step("获取测试URI值。"):
                cls.URI = cls.config.getItem('uri', 'INAuth')
            ...
            with allure.step("user register."):
                json = {"code_type": 0, "client_type": 1, "client_version": "v1", "device_token": "123456789",
                        "imei": "460011234567890", "phone": "13511222232", "sms_code": "123456",
    ...
    def test_123001_user_auth_correct(self):
        """ Test user auth by correct parameters(FT-HTJK-123-001)."""
        self.logger.info(".... Start test_123001_user_auth_correct ....")
        try:
            ...
            self.logger.info(".... End test_123001_user_auth_correct ....")
            self.logger.info("")

    @allure.severity("blocker")
    @allure.story("未认证获取授权")
    @allure.testcase("FT-HTJK-123-002")
    def test_123002_user_auth_without_identity(self):
        """ Test user auth without identity(FT-HTJK-123-002)."""
        self.logger.info(".... Start test_123002_user_auth_without_identity ....")
        try:
            ...
            with allure.step("teststep: delete user identity record"):
                ...
                allure.attach("delete result", str(select_result))
                self.logger.info("delete result: {0}".format(select_result))
            self.logger.info(".... End test_123002_user_auth_without_identity ....")
            self.logger.info("")
