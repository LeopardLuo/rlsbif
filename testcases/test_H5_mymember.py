...
class TestMyMember(object):
    ...
    @allure.story("查询访客服务单")
    @allure.testcase("FT-HTJK-xxx-xxx")
    def test_mymember_visitor_order(self):
        """ Test mymember with visitor order. """
        self.logger.info(".... Start test_mymember_visitor_order ....")
        try:
            ...
            with allure.step("teststep12: delete order."):
                ...
                allure.attach("service order list", str(r_memlist))
                self.logger.info("service order list: {0}".format(r_memlist))
                assert r_memlist
            ...
            with allure.step("teststep: delete insert user info"):
                ...
                allure.attach("delete result", str(delete_result))
                self.logger.info("delete result: {0}".format(delete_result))
            self.logger.info(".... End test_mymember_visitor_order ....")
            self.logger.info("")


if __name__ == '__main__':
    pytest.main(['-s', 'test_H5_mymember.py::TestMyMember::test_mymember_visitor_order'])