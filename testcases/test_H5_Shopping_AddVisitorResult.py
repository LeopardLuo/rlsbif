...
class TestShoppingAddVisitorResult(object):
    ...
    @allure.severity("critical")
    @allure.story("在白名单下重复成员访客服务单")
    @allure.testcase("FT-HTJK-203-025")
    def test_203025_repeat_add_visitor_result_other_with_whitelist(self):
        """ Test repeat add visitor result interface add other with whitelist"""
        self.logger.info(".... Start test_203025_repeat_add_visitor_result_other_with_whitelist ....")
        try:
            with allure.step("teststep1: user register."):
                json = {"code_type": 0, "client_type": 1, "client_version": "v1", "device_token": "12345678901" * 4,
                        "imei": "460011234567890", "phone": "13511229000", "sms_code": "123456",
            ...
            with allure.step("teststep: delete insert user info"):
                ...
                allure.attach("delete result", str(delete_result))
                self.logger.info("delete result: {0}".format(delete_result))
            self.logger.info(".... End test_203025_repeat_add_visitor_result_other_with_whitelist ....")
            self.logger.info("")
